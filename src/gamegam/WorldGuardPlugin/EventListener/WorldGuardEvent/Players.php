...
class Players implements Listener{
	...
	public function onLaunch(ProjectileLaunchEvent $event): void
	{
		...
		if ($p instanceof Player && $projectile instanceof EnderPearl) {
			$pos = $p->getPosition();
			$guard = WorldData::getInstance();
	...
	public function onMove(PlayerMoveEvent $ev)
	{
		...
		if (
			$guard->getBlockJoin($pos) &&
			$d->getEntry($name) &&
			! Server::getInstance()->isOp($p->getName()) &&
			! $from->floor()->equals($to->floor()) &&
			!$d->getMembers($name, $p->getName())
		) {
			if ($guard->getName($from) !== $name) {
				$ev->cancel();
			}
		}
	...
	public function onQuit(PlayerQuitEvent $ev){
		unset($this->player[$ev->getPlayer()->getName()]);
	}
}