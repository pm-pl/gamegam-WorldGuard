...
namespace gamegam\WorldGuardPlugin\EventListener\WorldGuardEvent;
	...
	public function onDamage(EntityDamageEvent $ev){
		...
		if ($entity instanceof Player){
			if ($data->getBlockJoin($pos)){
				...
				if ($d->getinvincible($name)){
					...
					if ($d->getFlag($name, "fall-damage")){
						if ($cause == 4){
							$ev->cancel();
						}
					}
				}
                if ($da->getinvincible($entity->getWorld()->getFolderName())){
                    $ev->cancel();
                }
                if ($da->getFlag($entity->getWorld()->getFolderName(), "fall-damage")){
                    if ($cause == 4){
                        $ev->cancel();
                    }
                }
		...
		if ($ev instanceof EntityDamageByEntityEvent){
			...
			if ($p instanceof Player && $entity instanceof Player){
                ...
                if ($da->getPVP($entity->getWorld()->getFolderName())){
                    $ev->cancel();
                }
			}
			if ($entity instanceof Player && ! $p instanceof Player){
                ...
                if ($da->getMobDamage($entity->getWorld()->getFolderName())){
                    $ev->cancel();
                }
			}
