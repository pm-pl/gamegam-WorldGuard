                    ...
                    case "info";
                        ...
                        if ($isi == 0){
                            ...
                            if ($isi == 2){
                                // world info
                                $flag = [];
                                foreach ($this->api->worlds["name"][$args[1]]["flag"] as $list => $bool) {
                                    $flag[] = $list;
                                }
                                $flag = implode(", ", $flag);
                                $str = str_replace("(region)", $args[1], $this->api->getAPI()->getString("list"));
                                $str = str_replace("(flag)", $flag, $str);
                                $str = str_replace("(members)", "(none)", $str);
                                $str = str_replace("(pos1)", "(none)", $str);
                                $str = str_replace("(pos2)", "(none)", $str);
                                $str = str_replace("(world)", $args[1] , $str);
                                $p->sendMessage($str);
                            }
                        }
                        break;
